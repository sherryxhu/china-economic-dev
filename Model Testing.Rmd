...
m2=lmer(log(THOUSANDS_SHIFTED) ~ CYEAR + URBAN +  
          CYEAR*URBAN + CYEAR*T1 + URBAN*T1
          + (1 + CYEAR|HHID) + (1|T1), REML = FALSE, data = df)
summary(m2)
anova(m2)
```
...
for(i in 1:n_times){
  ...
  our_model = lmer(log(THOUSANDS_SHIFTED) ~ CYEAR + URBAN 
            + CYEAR * PRIMARY
            + CYEAR * URBAN    
            + (1+CYEAR+URBAN+PRIMARY | T1)
          + (1 + CYEAR|HHID), REML = FALSE, data = train_fold)
