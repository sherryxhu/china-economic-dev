...
m1_formula = as.formula("LOG_THOUSANDS_SHIFTED ~ CYEAR + T1 +  URBAN + BUSINESS_IND + FARM_IND + FISH_IND +  LVST_IND + OTHR_IND + RETIRE_IND + SUB_IND + (1|HHID)")
m1 = lmer( m1_formula, data = df_trimmed)
summary(m1)
```
...
# remaining_variables_of_interest = variables_of_interest
# 
# current_variables = c("CYEAR", "URBAN",
# "T1",
# "BUSINESS_IND",
# "FARM_IND",
# "FISH_IND",
...
# three_way_model
three_way_model_formula = as.formula("LOG_THOUSANDS_SHIFTED ~ CYEAR + URBAN + T1 + BUSINESS_IND + FARM_IND + 
    FISH_IND + LVST_IND + OTHR_IND + RETIRE_IND + SUB_IND + (1 | 
...
for(i in 1:test_size){
  ...
  initial_model = lmer(m1_formula, REML=FALSE, data=train_fold)
  predictions = predict(initial_model, test_fold, allow.new.levels=TRUE)
  residuals = exp(predictions) - exp(test_fold$LOG_THOUSANDS_SHIFTED)
  MSE = mean(residuals^2)
  initial_MSEs = c(initial_MSEs, MSE)
...
```{r}
for (i in yrs) {
  prov21 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df7 <- data.frame(prov23)
}
for (i in yrs) {
  prov23 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_23_IND1",]+yrs*estimate["CYEAR:T1_23_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df8 <- data.frame(prov21)
}
for (i in yrs) {
  prov32 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_32_IND1",]+yrs*estimate["CYEAR:T1_32_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df10 <- data.frame(prov32)
}
for (i in yrs) {
  prov37 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_37_IND1",]+yrs*estimate["CYEAR:T1_37_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df11 <- data.frame(prov37)
}
for (i in yrs) {
  prov41 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_41_IND1",]+yrs*estimate["CYEAR:T1_41_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df12 <- data.frame(prov41)
}
for (i in yrs) {
  prov42 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_42_IND1",]+yrs*estimate["CYEAR:T1_42_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df13 <- data.frame(prov42)
}
for (i in yrs) {
  prov43 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_43_IND1",]+yrs*estimate["CYEAR:T1_43_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df14 <- data.frame(prov43)
}
for (i in yrs) {
  prov45 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_45_IND1",]+yrs*estimate["CYEAR:T1_45_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
  df15 <- data.frame(prov45)
}
for (i in yrs) {
  prov52 <- exp(estimate["(Intercept)",]+yrs*estimate["CYEAR",]+estimate["T1_52_IND1",]+yrs*estimate["CYEAR:T1_52_IND1",]) - (abs(min(df_trimmed$THOUSANDS))+0.01)
