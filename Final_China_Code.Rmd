...
```{r}
library(gridExtra)
coefs <- names(fixef(our_model))
#year
coeftest_year <- linearHypothesis(our_model, "CYEAR=0")
#urban/rural
coeftest_urban <- linearHypothesis(our_model, "URBAN1=0")
#province
coeftest_province <- linearHypothesis(our_model, coefs[4:14])
#primary
coeftest_primary <- linearHypothesis(our_model, coefs[15:21])
#year:primary
coeftest_yearprimary <- linearHypothesis(our_model, coefs[22:28])
#year:urban
coeftest_yearurban <- linearHypothesis(our_model, coefs[29])
#year:province
coeftest_yearprovince <- linearHypothesis(our_model, coefs[30:40])
#urban:province
coeftest_urbanprovince <- linearHypothesis(our_model, coefs[41:51])
...
estimate <- as.data.frame(summary(our_model)$coefficients)
...
plot2 <- ggplot(mdf2, aes(x=xvar, y=value, group=variable, color=variable)) + 
  ...
  labs(title="Variation in Household Income by Province",
       x="Year",
       y="Beta coefficients")
```
...
plot3 <- ggplot(mdf3, aes(x=xvar, y=value, group=variable, color=variable)) +
  ...
  labs(title="Variation in Household Income by Urban/Rural Status",
       x="Year",
       y="Beta coefficients")
```
...
plot1 <- ggplot(mdf1, aes(x=xvar, y=value, group=variable, color=variable)) + 
  ...
  labs(title="Variation in Household Income Source",
       x="Year",
       y="Beta coefficients")
```